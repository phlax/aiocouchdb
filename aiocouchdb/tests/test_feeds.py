...
class FeedTestCase(utils.TestCase):
    ...
    async def test_ignore_empty_chunks(self):
    ...
    async def test_read_empty_json_feed(self):
    ...
    async def test_calling_inactive_feed_returns_none(self):
        resp = self.prepare_response()
    ...
    async def test_close_resp_on_feed_end(self):
        resp = self.prepare_response()
    ...
    async def test_close_resp_on_error(self):
    ...
    # BROKEN: this test needs to wait for the loop to initialize
    # and the the buffer to be filled. Not sure how to do that
    async def __test_buffer_workflow(self):
        resp = self.prepare_response(data=[
            b'foo\r\n', b'bar\r\n',
            b'baz\r\n', b'boo\r\n'
        ])
...
class JsonFeedTestCase(utils.TestCase):
    ...
    async def test_read_json_chunks(self):
        resp = self.prepare_response(data=[
            b'{"foo": true}\r\n',
            b'"foo"\r\n',
            b'{"bar": null}\r\n'
...
class ViewFeedTestCase(utils.TestCase):
    ...
    async def test_read_empty_view(self):
        resp = self.prepare_response(data=[
    ...
    async def test_read_reduced_empty_view(self):
        resp = self.prepare_response(data=[
            b'{"rows": [\r\n',
            b'\r\n]}'
        ])
    ...
    async def test_view_header(self):
        resp = self.prepare_response(data=[
            b'{"total_rows": 3, "offset": 0, "rows": [\r\n',
            b'{"id": "foo", "key": null, "value": false}',
            b',\r\n{"id": "bar", "key": null, "value": false}',
    ...
    async def test_reduced_view_header(self):
        resp = self.prepare_response(data=[
            b'{"rows": [\r\n',
            b'{"key": null, "value": 1}',
            b',\r\n{"key": true, "value": 2}',
...
class EventSourceFeedTestCase(utils.TestCase):
    ...
    async def test_read_empty_data(self):
        resp = self.prepare_response(data=[
            b'event: heartbeat\ndata: \n\n'
        ])
    ...
    async def test_read_multiple_data(self):
        resp = self.prepare_response(data=[
            b'data: [\ndata:"foo",\ndata: "bar"\ndata:] \n\n'
        ])
    ...
    async def test_leading_colon(self):
        resp = self.prepare_response(data=[
            b':id\n\n'
        ])
    ...
    async def test_decode_retry_with_int(self):
        resp = self.prepare_response(data=[
            b'retry: 10\n\n'
        ])
    ...
    async def test_ignore_unknown_field(self):
        resp = self.prepare_response(data=[
            b'data: "foo"\nfoo: bar\n\n'
        ])
...
class ChangesFeedTestCase(utils.TestCase):
    ...
    async def test_read_changes(self):
        resp = self.prepare_response(data=[
            b'{"results":[\n',
        ...
        await self.check_feed_output(feed, self.output)

    async def test_read_changes_longpoll(self):
        resp = self.prepare_response(data=[
            b'{"results":[\n',
            b'{"seq":77,"id":"foo","changes":[{"rev":"9-CDE"}]}',
            b',\n{"seq":90,"id":"bar","changes":[{"rev":"12-ABC"}]}',
    ...
    async def test_read_changes_continuous(self):
        resp = self.prepare_response(data=[
            b'{"seq":77,"id":"foo","changes":[{"rev":"9-CDE"}]}\n',
    ...
    async def test_read_changes_eventsource(self):
        resp = self.prepare_response(data=[
    ...
    async def check_feed_output(self, feed, output):
        self.assertTrue(feed.is_active())
