...
class ServerConfigTestCase(utils.ServerTestCase):
    ...
    async def test_config(self):
        await self.server.config.get()
        self.assert_request_called_with('GET', '_config')

    async def test_config_get_section(self):
        await self.server.config.get('couchdb')
        self.assert_request_called_with('GET', '_config', 'couchdb')

    async def test_config_get_option(self):
        await self.server.config.get('couchdb', 'uuid')
        self.assert_request_called_with('GET', '_config', 'couchdb', 'uuid')

    @utils.modify_server('aiocouchdb', 'test', 'relax')
    async def test_config_set_option(self):
        with self.response(data=b'"relax!"'):
            result = await self.server.config.update(
    ...
    async def test_config_del_option(self):
        with self.response(data=b'"passed"'):
            result = await self.server.config.delete('aiocouchdb', 'test')
            self.assert_request_called_with('DELETE',
                                            '_config', 'aiocouchdb', 'test')
        self.assertIsInstance(result, str)

    async def test_config_option_exists(self):
        with self.response(status=200):
            result = await self.server.config.exists('couchdb', 'uuid')
            self.assert_request_called_with('HEAD', '_config',
                                            'couchdb', 'uuid')
            self.assertTrue(result)

    async def test_config_option_not_exists(self):
        with self.response(status=404):
            result = await self.server.config.exists('foo', 'bar')
