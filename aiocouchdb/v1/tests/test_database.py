...
from aiocouchdb.v1.security import DatabaseSecurity
...
class DatabaseTestCase(utils.DatabaseTestCase):
    ...
    async def test_init_with_name_from_server(self):
        server = aiocouchdb.v1.server.Server()
        db = await server.db('foo')
        self.assertEqual(db.name, 'foo')

    async def test_exists(self):
        result = await self.db.exists()
    ...
    async def test_exists_forbidden(self, root):
        await self.db.security.update_members(auth=root, names=['foo'])
    ...
    async def test_all_docs_params(self):
        ...
        for key, value in all_params.items():
            await self.db.all_docs(**{key: value})
            if key in ('endkey', 'startkey'):
                value = json.dumps(value)
            self.assert_request_called_with('GET', self.db.name, '_all_docs',
                                            params={key: value})

    async def test_all_docs_key(self):
        with (await self.db.all_docs('foo')):
            self.assert_request_called_with('GET', self.db.name, '_all_docs',
                                            params={'key': '"foo"'})

    async def test_all_docs_keys(self):
        with (await self.db.all_docs('foo', 'bar', 'baz')):
            self.assert_request_called_with(
                'POST', self.db.name, '_all_docs',
                data={'keys': ('foo', 'bar', 'baz')})

    async def test_bulk_docs(self):
        await self.db.bulk_docs([{'_id': 'foo'}, {'_id': 'bar'}])
        self.assert_request_called_with('POST', self.db.name, '_bulk_docs',
                                        data=Ellipsis)
        data = self.request.call_args[1]['data']

        # cython created generators dont seem to type validate
        assert str(type(data)) == "<class 'generator'>"
    ...
    async def test_bulk_docs_all_or_nothing(self):
        await self.db.bulk_docs([{'_id': 'foo'}, {'_id': 'bar'}],
                                     all_or_nothing=True)
        self.assert_request_called_with(
            'POST', self.db.name, '_bulk_docs',
            data=Ellipsis)
        data = self.request.call_args[1]['data']
    ...
    async def test_bulk_docs_new_edits(self):
        await self.db.bulk_docs([{'_rev': '1-foo'}], new_edits=False)
        self.assert_request_called_with('POST', self.db.name, '_bulk_docs',
                                        data=Ellipsis,
                                        params={})

    async def test_changes(self):
        with (await self.db.changes()) as feed:
            self.assertIsInstance(feed, aiocouchdb.feeds.ChangesFeed)
            self.assert_request_called_with('GET', self.db.name, '_changes')

    @utils.skip_for('mock')
    async def test_changes_reading(self):
        ids = [utils.uuid() for _ in range(3)]

        for idx in ids:
            await self.db[idx].update({})
    ...
    async def test_changes_longpoll(self):
        with (await self.db.changes(feed='longpoll')) as feed:
            self.assertIsInstance(feed, aiocouchdb.feeds.LongPollChangesFeed)
            self.assert_request_called_with(
                'GET', self.db.name, '_changes',
                params={'feed': 'longpoll'})

    async def test_changes_continuous(self):
        with (await self.db.changes(feed='continuous')) as feed:
            self.assertIsInstance(feed, aiocouchdb.feeds.ContinuousChangesFeed)
            self.assert_request_called_with('GET', self.db.name, '_changes',
                                            params={'feed': 'continuous'})

    @utils.skip_for('mock')
    async def test_changes_continuous_reading(self):
        ids = [utils.uuid() for _ in range(3)]
        ...
        with (await self.db.changes(feed='continuous',
    ...
    async def test_changes_eventsource(self):
        with (await self.db.changes(feed='eventsource')) as feed:
    ...
    async def test_changes_eventsource(self):
        ...
        with (await self.db.changes(feed='eventsource',
                                         timeout=1000)) as feed:
    ...
    async def test_changes_doc_ids(self):
        with (await self.db.changes('foo', 'bar')):
            self.assert_request_called_with('POST', self.db.name, '_changes',
                                            data={'doc_ids': ('foo', 'bar')},
                                            params={'filter': '_doc_ids'})

    async def test_changes_assert_filter_doc_ids(self):
        with self.assertRaises(AssertionError):
            await self.db.changes('foo', 'bar', filter='somefilter')

    @utils.skip_for('mock')
    async def test_changes_filter_docid(self):
        ids = [utils.uuid() for _ in range(100)]
        filtered_ids = [random.choice(ids) for _ in range(10)]

        await self.db.bulk_docs({'_id': idx} for idx in ids)

        with (await self.db.changes(*filtered_ids)) as feed:
    ...
    @utils.skip_for('mock')
    async def test_changes_filter_view(self):
        docs = await utils.populate_database(self.db, 10)
        expected = [doc['_id'] for doc in docs.values() if doc['num'] > 5]
        ddoc = self.db['_design/' + utils.uuid()]

        await ddoc.doc.update({
            'views': {
                'test': {
        ...
        with (await self.db.changes(view=view_name)) as feed:
    ...
    @utils.run_for('mock')
    async def test_changes_params(self):
        all_params = {
        ...
        for key, value in all_params.items():
            await self.db.changes(**{key: value})
            headers = {}
            if key == 'params':
                params = value
            elif key == 'headers':
    ...
    async def test_compact_ddoc(self):
        resp = await self.db.resource.put(dict(path='_design/ddoc', data={}))
        resp.close()

        await self.db.compact('ddoc')
        self.assert_request_called_with(
            'POST', self.db.name, '_compact', 'ddoc')

    async def test_document(self):
        result = await self.db.doc('docid')
        self.assert_request_called_with('HEAD', self.db.name, 'docid')
        self.assertIsInstance(result, self.db.document_class)

    async def test_document_custom_class(self):
        ...
        result = await db.doc('docid')
        self.assert_request_called_with('HEAD', self.db.name, 'docid')
        self.assertIsInstance(result, CustomDocument)
        self.assertIsInstance(result.resource, aiocouchdb.client.Resource)

    async def test_document_docid_gen_fun(self):
        def custom_id():
            return 'foo'

        result = await self.db.doc(idfun=custom_id)
        self.assert_request_called_with('HEAD', self.db.name, 'foo')
        self.assertIsInstance(result, self.db.document_class)

    async def test_document_docid_gen_fun_default_uuid(self):
        result = await self.db.doc()
    ...
    async def test_design_document_custom_class(self):
        ...
        result = await db.ddoc('_design/ddoc')
        self.assert_request_called_with('HEAD', self.db.name, '_design', 'ddoc')
        self.assertIsInstance(result, CustomDocument)
        self.assertIsInstance(result.resource, aiocouchdb.client.Resource)

    async def test_document_get_item(self):
        doc = self.db['docid']
        with self.assertRaises(AssertionError):
            self.assert_request_called_with('HEAD', self.db.name, 'docid')
        self.assertIsInstance(doc, self.db.document_class)

    async def test_design_document_get_item(self):
    ...
    async def test_ensure_full_commit(self):
        await self.db.ensure_full_commit()
        self.assert_request_called_with(
            'POST', self.db.name, '_ensure_full_commit')

    async def test_missing_revs(self):
        await self.db.missing_revs({'docid': ['1-rev', '2-rev']})
        self.assert_request_called_with('POST', self.db.name, '_missing_revs',
                                        data={'docid': ['1-rev', '2-rev']})

    async def test_purge(self):
        await self.db.purge({'docid': ['1-rev', '2-rev']})
        self.assert_request_called_with('POST', self.db.name, '_purge',
                                        data={'docid': ['1-rev', '2-rev']})

    async def test_revs_diff(self):
        await self.db.revs_diff({'docid': ['1-rev', '2-rev']})
        self.assert_request_called_with('POST', self.db.name, '_revs_diff',
                                        data={'docid': ['1-rev', '2-rev']})

    async def test_revs_limit(self):
        await self.db.revs_limit()
        self.assert_request_called_with('GET', self.db.name, '_revs_limit')

    async def test_revs_limit_update(self):
        await self.db.revs_limit(42)
    ...
    def test_security_custom_class(self):
        class CustomSecurity(DatabaseSecurity):
    ...
    async def test_temp_view(self):
        mapfun = 'function(doc){ emit(doc._id); }'
        with (await self.db.temp_view(mapfun)) as view:
            self.assert_request_called_with('POST', self.db.name, '_temp_view',
                                            data={'map': mapfun})
            self.assertIsInstance(view, aiocouchdb.feeds.ViewFeed)

    async def test_temp_view_reduce(self):
        mapfun = 'function(doc){ emit(doc._id); }'
        redfun = '_count'
        with (await self.db.temp_view(mapfun, redfun)) as view:
            self.assert_request_called_with('POST', self.db.name, '_temp_view',
                                            data={'map': mapfun,
                                                  'reduce': redfun})
            self.assertIsInstance(view, aiocouchdb.feeds.ViewFeed)

    async def test_temp_view_language(self):
        mapfun = 'function(doc){ emit(doc._id); }'
        with (await self.db.temp_view(mapfun, language='javascript')):
            self.assert_request_called_with('POST', self.db.name, '_temp_view',
                                            data={'map': mapfun,
                                                  'language': 'javascript'})

    async def test_temp_view_startkey_none(self):
        mapfun = 'function(doc){ emit(doc._id); }'

        with (await self.db.temp_view(mapfun, startkey=None)):
            self.assert_request_called_with('POST', self.db.name, '_temp_view',
                                            data={'map': mapfun},
                                            params={'startkey': 'null'})

    async def test_temp_view_endkey_none(self):
        mapfun = 'function(doc){ emit(doc._id); }'

        with (await self.db.temp_view(mapfun, endkey=None)):
            self.assert_request_called_with('POST', self.db.name, '_temp_view',
                                            data={'map': mapfun},
                                            params={'endkey': 'null'})

    @utils.run_for('mock')
    async def test_temp_view_params(self):
        all_params = {
            'att_encoding_info': False,
            'attachments': False,
            'conflicts': True,
        ...
        for key, value in all_params.items():
            await self.db.temp_view('fun(_)-> ok end', **{key: value})
            if key in ('endkey', 'startkey'):
                value = json.dumps(value)
            if key == 'keys':
                self.assert_request_called_with(
    ...
    async def test_view_cleanup(self):
        await self.db.view_cleanup()
