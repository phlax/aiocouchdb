...
cdef class DatabaseSecurity(object):
    ...
    async def update(self, *, auth=None, admins=None, members=None, merge=False):
        ...
        """
        secobj = await self.get(auth=auth)
        ...
        resp = await self.resource.put(dict(auth=auth, data=secobj))
